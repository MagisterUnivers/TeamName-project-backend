...
const register = async (req, res) => {
	...
	const sendEmailToVerify = {
		...
		html: `<a href="${BASE_URL}/register?verificationToken=${verificationToken}">Click to verify email</a>`
	};
...
const updateUser = async (req, res) => {
	...
	if (file) {
		...
		updatedFields.avatarURL = req.user.avatarURL;
	}

	if (name) {
		updatedFields.name = name;
	} else {
		updatedFields.name = req.user.name;
	}

	updatedFields.email = req.user.email;
	updatedFields.id = req.user._id;

	if (Object.keys(updatedFields).length > 0) {
		await req.user.updateOne(updatedFields, { new: true });
	}
